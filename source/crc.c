#include "crc.h"
#include "log.h"

#include <stddef.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define BUFFER 4096

static inline uint32_t crc8(unsigned crc, unsigned char const *data,
                            size_t len);

static unsigned char const crc8_table[] = {
    };

static inline uint32_t crc8(uint32_t crc, unsigned char const *data,
                            size_t len) {
  if (data == NULL)
    return 0;
  crc &= 0xff;
  unsigned char const *end = data + len;
  while (data < end)
    crc = crc8_table[crc ^ *data++];
  return crc;
}

uint32_t get_crc(const char *path_to_file) {
  FILE *file = fopen(path_to_file, "rb");
  if (file == NULL) {
    print_error("Failed to open a file to get crc.");
    return 0;
  }
  uint32_t crc = 0;
  unsigned char buffer[BUFFER];
  size_t readed;

  while (1) {
    readed = fread(buffer, 1, BUFFER, file);
    if (readed == 0)
      break;
    crc = crc8(crc, buffer, readed);
  }
  fclose(file);
  return crc;
}
